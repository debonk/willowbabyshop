...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		if ($this->config->get('cashback_status')) {
			...
			$data['cashback_status'] = false;
		}
		//Bonk04:End

		foreach ($results as $result) {
			//Bonk05
			$reward_total = $this->model_customer_customer->getTotalCustomerRewardsByOrderId($result['order_id']);
			...
			if ($this->config->get('cashback_status')) {
				if (strtotime($result['date_added']) > $cashback_date_start) {
					...
					foreach ($totals as $total) {
						if ($total['code'] == 'sub_total' || $total['code'] == 'voucher' || $total['code'] == 'credit' || $total['code'] == 'reward' || $total['code'] == 'coupon') {
							$subtotal += $total['value'];
						}
					}

					if ($subtotal >= $cashback_min_total) {
						if ($subtotal > $cashback_max_total) {
							$subtotal = $cashback_max_total;
						}
						$cashback_point = floor($subtotal * $cashback_discount);
					} else {
						$cashback_point = 0;
					}
				} else {
	...
	public function getForm() {
		...
		if (!empty($order_info)) {
			...
			$data['payment_city_name'] = $order_info['payment_city_name']; // Bonk
			...
			$data['shipping_city_name'] = $order_info['shipping_city_name']; // Bonk
...
*/
			...
			$replace = array(
				...
				'city_name' => $order_info['payment_city_name'], // Bonk
			...
			$replace = array(
				...
				'city_name' => $order_info['shipping_city_name'], //Bonk
			...
			$complete_status_id = $this->config->get('config_complete_status');
			if (in_array($order_info['order_status_id'], $complete_status_id)) {
	...
	public function addCashback() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($order_info && $order_info['customer_id'] && $cashback_point) {
				...
				if (!$cashback_total) {
					$cashback_data = array(
						'code'  => $order_id . 'C' . token(9 - strlen($order_id)),
						'from_name' => 'Willow Baby & Kids',
						'from_email' => 'contact@willowbabyshop.com',
						'to_name'  => $order_info['customer'],
						'to_email'  => $order_info['email'],
						'voucher_theme_id'  => $this->config->get('cashback_voucher_theme_id'),
						'message'  => sprintf($this->language->get('text_cashback_desc'), $order_id, date($this->language->get('date_format_short'), strtotime('+' . $this->config->get('cashback_valid') . 'days'))),
						'amount'  => $cashback_point,
						'status' => 0
					);
