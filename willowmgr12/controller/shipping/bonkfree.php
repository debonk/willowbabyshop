<?php
class ControllerShippingBonkFree extends Controller {
	private $error = array();
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('bonkfree', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('shipping/bonkfree', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('shipping/bonkfree', 'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('extension/shipping', 'token=' . $this->session->data['token'], true);

		if (isset($this->request->post['bonkfree_multizone'])) {
			$data['bonkfree_multizone'] = $this->request->post['bonkfree_multizone'];
		} else {
		...
		if (isset($this->request->post['bonkfree_status'])) {
			...
			$data['bonkfree_status'] = $this->config->get('bonkfree_status');
		}

		if (isset($this->request->post['bonkfree_sort_order'])) {
			$data['bonkfree_sort_order'] = $this->request->post['bonkfree_sort_order'];
		} else {
			$data['bonkfree_sort_order'] = $this->config->get('bonkfree_sort_order');
		}
		...
		$this->response->setOutput($this->load->view('shipping/bonkfree', $data));
	}
