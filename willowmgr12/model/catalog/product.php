...
class ModelCatalogProduct extends Model
{
	public function addProduct($data)
	{
		...
		$option_ids = isset($data['product_multiple']['option']) ? $data['product_multiple']['option'] : [];

		$this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_id = '" . (int)$product_id . "', option_id = '" . $this->db->escape(json_encode($option_ids)) . "', value = '', required = '1'");

		$product_option_id = $this->db->getLastId();

		foreach ($data['product_multiple']['multiple_value'] as $product_option_value) {
			$option_value_ids = isset($product_option_value['option_value_id']) ? $product_option_value['option_value_id'] : [];

			$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . $this->db->escape(json_encode($option_ids)) . "', option_value_id = '" . $this->db->escape(json_encode($option_value_ids)) . "', model = '" . $this->db->escape($product_option_value['model']) . "', image = '" . $this->db->escape($product_option_value['image']) . "', quantity = '" . (int)$product_option_value['quantity'] . "', price = '" . (float)$product_option_value['price'] . "', points = '" . (int)$product_option_value['points'] . "', weight = '" . (float)$product_option_value['weight'] . "', weight_class_id = '" . (int)$product_option_value['weight_class_id'] . "'");
		}
	...
	public function editProductDelete($product_id, $data)
	{
	...
	public function copyProduct($product_id)
	{
		...
		if ($query->num_rows) {
			...
			$data['product_multiple'] = $this->getProductMultiple($product_id);

			if ($data['product_multiple']) {
				foreach ($data['product_multiple']['multiple_value'] as $key => $multiple_value) {
					$data['product_multiple']['multiple_value'][$key]['model'] .= '-' . $token;

					$extension = '.' . pathinfo($multiple_value['image'], PATHINFO_EXTENSION);
					$new_image = str_replace($extension, '-' . $token . $extension, $multiple_value['image']);

					copy(DIR_IMAGE . $multiple_value['image'], DIR_IMAGE . $new_image);

					$data['product_multiple']['multiple_value'][$key]['image'] = $new_image;
				}
			}
	...
	public function deleteProduct($product_id)
	{
		...
		$product_multiple = $this->getProductMultiple($product_id);

		$images = array_merge($images, array_column($product_multiple['multiple_value'], 'image'));
	...
	public function getProductsDelete($data = array())
	{
		$sql = "SELECT p.*, pd.*, p2c.category_id, m.name AS manufacturer_name FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";

		if (!empty($data['filter_model'])) {
			$sql .= " AND (p.model LIKE '%" . $this->db->escape($data['filter_model']) . "%' OR p.product_id IN (SELECT DISTINCT product_id FROM " . DB_PREFIX . "product_option_value WHERE model LIKE '%" . $this->db->escape($data['filter_model']) . "%'))";
		}
	...
	public function getProductMultiple($product_id)
	{
		...
		if ($product_option_query->num_rows) {
			...
			$option_ids = json_decode($product_option_query->row['option_id']);

			if (is_array($option_ids)) {
				$option_data = $option_ids;

				$product_multiple_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option_value WHERE product_option_id = '" . (int)$product_option_query->row['product_option_id'] . "'");

				foreach ($product_multiple_value_query->rows as $product_multiple_value) {
					$multiple_value_data[] = array(
						'product_option_value_id' => $product_multiple_value['product_option_value_id'],
						'option_value_id'         => json_decode($product_multiple_value['option_value_id']),
						'model'               	  => $product_multiple_value['model'],
						'image'               	  => $product_multiple_value['image'],
						'quantity'                => $product_multiple_value['quantity'],
						'price'                   => $product_multiple_value['price'],
						'points'                  => $product_multiple_value['points'],
						'weight'                  => $product_multiple_value['weight'],
						'weight_class_id'         => $product_multiple_value['weight_class_id']
					);
				}

				$product_multiple_data = array(
					'option'			=> $option_data,
		...
		return $product_multiple_data;
	}

	public function addProductOption($product_id, $data)
	{
		if (isset($data['product_option'])) {
			if ($data['product_option']['type'] == 'select' || $data['product_option']['type'] == 'radio' || $data['product_option']['type'] == 'checkbox' || $data['product_option']['type'] == 'image') {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_id = '" . (int)$product_id . "', option_id = '" . (int)$data['product_option']['option_id'] . "', required = '" . (int)$data['product_option']['required'] . "'");
			} else {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_id = '" . (int)$product_id . "', option_id = '" . (int)$data['product_option']['option_id'] . "', value = '" . $this->db->escape($data['product_option']['value']) . "', required = '" . (int)$data['product_option']['required'] . "'");
			}

			$product_option_id = $this->db->getLastId();
		}

		if ($data['product_option_value']['product_option_id']) {
			$product_option_id = $data['product_option_value']['product_option_id'];
		}

		$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$data['product_option_value']['option_id'] . "', option_value_id = '" . (int)$data['product_option_value']['option_value_id'] . "', model = '" . $this->db->escape($data['product_option_value']['model']) . "', quantity = '" . (int)$data['product_option_value']['quantity'] . "', subtract = '" . (int)$data['product_option_value']['subtract'] . "', price = '" . (float)$data['product_option_value']['price'] . "', price_prefix = '" . $this->db->escape($data['product_option_value']['price_prefix']) . "', points = '" . (int)$data['product_option_value']['points'] . "', points_prefix = '" . $this->db->escape($data['product_option_value']['points_prefix']) . "', weight = '" . (float)$data['product_option_value']['weight'] . "', weight_prefix = '" . $this->db->escape($data['product_option_value']['weight_prefix']) . "'");
	}
	...
	{
		$sql = "SELECT COUNT(DISTINCT pov.product_option_value_id) AS total FROM `oc_product_option_value` pov LEFT JOIN oc_product p ON (pov.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) LEFT JOIN " . DB_PREFIX . "user u ON (p.user_id = u.user_id) ";
	...
	public function getTotalProductsDelete($data = array())
	{
