...
class ModelCatalogOption extends Model
{
	...
	public function getOptions($data = array())
	{
		...
		if (!empty($data['filter_type'])) {
			$sql .= " AND o.type IN ('" . stripslashes($this->db->escape(implode('\', \'', $data['filter_type']))) . "')";
		}

		if (isset($data['filter_selection_only'])) {
			if (!empty($data['filter_selection_only'])) {
				$sql .= " AND o.type IN ('select', 'radio', 'checkbox', 'image')";
			} else {
				$sql .= " AND o.type NOT IN ('select', 'radio', 'checkbox', 'image')";
			}
		}
	...
	public function getOptionValues_Delete($option_id)
	{
		$option_value_data = array();
	...
	public function getOptionValueDescriptions($option_id)
	{
		...
		$option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "option_value ov LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ovd.option_value_id = ov.option_value_id) WHERE ov.option_id = '" . (int)$option_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY ov.sort_order, ovd.name");
	...
	public function getOptionValuesDescription($option_value_ids)
	{
		...
		return $option_value_description_query->rows;
	}
