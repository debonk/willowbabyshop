...
class ModelSaleVoucher extends Model {
	...
	//Bonk04
	public function getTotalVoucherByPrefixCode($code, $voucher_theme_id) {
		$query = $this->db->query("SELECT SUM(amount) AS total FROM " . DB_PREFIX . "voucher WHERE voucher_theme_id = '" . (int)$voucher_theme_id . "' AND code LIKE '" . $this->db->escape($code) . "%'");
	...
	public function sendVoucher($voucher_id) {
		...
		if ($voucher_info) {
			...
			// If voucher does not belong to an order
				...
				$data['text_greeting'] = sprintf($this->language->get('text_greeting'), $this->currency->format($voucher_info['amount'], $this->config->get('config_currency')));//Bonk00
