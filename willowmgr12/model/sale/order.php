...
class ModelSaleOrder extends Model {
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			// Bonk
			$city_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "city` WHERE city = '" . (int)$order_query->row['payment_city'] . "'");

			if ($city_query->num_rows) {
				$payment_city_name = $city_query->row['name'];
			} else {
				$payment_city_name = '';
			}
			...
			// Bonk
			$city_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "city` WHERE city = '" . (int)$order_query->row['shipping_city'] . "'");

			if ($city_query->num_rows) {
				$shipping_city_name = $city_query->row['name'];
			} else {
				$shipping_city_name = '';
			}
			...
			return array(
				...
				'payment_city_name'       => $payment_city_name, // Bonk
				...
				'shipping_city_name'      => $shipping_city_name, // Bonk
