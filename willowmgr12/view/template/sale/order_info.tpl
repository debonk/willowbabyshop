...
<!--                <td id="invoice" class="text-right"><?php //echo $invoice_no; ?></td>
    -->                
                ...
                <?php if ($invoice_no) { ?>
				  <?php echo $invoice_prefix . $invoice_no; ?>
				<?php } ?></td>
                <td style="width: 1%;" class="text-center">
				  <button id="button-invoice" data-loading-text="<?php echo $text_loading; ?>" data-toggle="tooltip" title="<?php echo $button_edit; ?>" class="btn btn-success btn-xs"><i class="fa fa-pencil"></i></button>
                </td>
              </tr>
              <tr>
                ...
                <td class="text-center"><?php if ($customer && $reward) { ?>
                  <?php if (!$reward_total) { ?>
					...
					<?php } else { ?>
					  <button disabled="disabled" data-toggle="tooltip" title="<?php echo $button_uncomplete_status; ?>" class="btn btn-success btn-xs"><i class="fa fa-plus-circle"></i></button>
					<?php } ?>
                  <?php } else { ?>
              ...
              <tr>
                <td><?php echo $text_cashback; ?></td>
                <td class="text-right"><?php echo $cashback; ?>
					<?php if ($error_cashback) { ?>
					  <span class="text-danger"><?php echo $error_cashback; ?></span>
					<?php } ?>
				</td>
                <td class="text-center"><?php if ($customer && $cashback_point) { ?>
                  <?php if (!$cashback_total) { ?>
					<?php if ($complete_status) { ?>
					  <button id="button-cashback-add" data-loading-text="<?php echo $text_loading; ?>" data-toggle="tooltip" title="<?php echo $button_cashback_add; ?>" class="btn btn-success btn-xs"><i class="fa fa-plus-circle"></i></button>
					<?php } else { ?>
					  <button disabled="disabled" data-toggle="tooltip" title="<?php echo $button_uncomplete_status; ?>" class="btn btn-success btn-xs"><i class="fa fa-minus-circle"></i></button>
					<?php } ?>
                  <?php } else { ?>
...
/* //Bonk11: function replaced by setinvoiceno
...
//Bonk11
$(document).delegate('#button-invoice', 'click', function() {
	$('#invoice').html('<?php echo $invoice_prefix; ?><input type="text" name="set_invoice_no" value="<?php echo $invoice_no; ?>" size="10" />');
	
	$('#button-invoice').replaceWith('<button id="button-set-invoice" data-loading-text="<?php echo $text_loading; ?>" data-toggle="tooltip" title="<?php echo $button_save; ?>" class="btn btn-success btn-xs"><i class="fa fa-floppy-o"></i></button>');
});

$(document).delegate('#button-set-invoice', 'click', function() {
	$.ajax({
		url: 'index.php?route=sale/order/setinvoiceno&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>',
		type: 'post',
		dataType: 'json',
		...
		success: function(json) {
			...
			if (json['invoice_no']) {
				...
				$('#button-set-invoice').replaceWith('<button id="button-invoice" data-loading-text="<?php echo $text_loading; ?>" data-toggle="tooltip" title="<?php echo $button_edit; ?>" class="btn btn-success btn-xs"><i class="fa fa-pencil"></i></button>');
			}
		},
		error: function(xhr, ajaxOptions, thrownError) {
			alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
...
$(document).delegate('#button-cashback-add', 'click', function() {
	$.ajax({
		url: 'index.php?route=sale/order/addcashback&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>',
		type: 'post',
		dataType: 'json',
		beforeSend: function() {
...
$('#button-history').on('click', function() {
	...
	$.ajax({
		...
		type: 'post',
		dataType: 'json',
		data: 'order_status_id=' + encodeURIComponent($('select[name=\'order_status_id\']').val()) + '&notify=' + ($('input[name=\'notify\']').prop('checked') ? 1 : 0) + '&override=' + ($('input[name=\'override\']').prop('checked') ? 1 : 0) + '&append=' + ($('input[name=\'append\']').prop('checked') ? 1 : 0) + '&comment=' + encodeURIComponent($('textarea[name=\'comment\']').val())  + '&user_id=<?php echo $user_id; ?>',//Bonk01
		beforeSend: function() {
