...
class ModelCheckoutOrder extends Model {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false, $user_id = 0) {
		...
		if ($order_info) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$order_status_id . "', notify = '" . (int)$notify . "', comment = '" . $this->db->escape($comment) . "', user_id = '" . (int)$user_id . "', date_added = NOW()"); //Bonk
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$mail->setFrom($this->config->get('config_email_2'));
				...
				if ($this->config->get('config_order_mail')) {
					...
					$mail->setTo($this->config->get('config_email_2'));
					$mail->setFrom($this->config->get('config_email_2'));
					$mail->setSender(html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'));
					$mail->setSubject(html_entity_decode($subject, ENT_QUOTES, 'UTF-8'));
					$mail->setHtml($this->load->view('mail/order', $data));
