...
class ControllerTotalVoucher extends Controller {
	...
	public function voucher() {
		...
		if (empty($this->request->post['voucher'])) {
			...
			//Bonk04
			$status = true;
			if ($voucher_info['voucher_theme_id'] == $this->config->get('cashback_voucher_theme_id')) {
				
//				$code_part = explode("C", $voucher_info['code']);
//				$voucher_prefix = $code_part[0];

				$date_expired = strtotime($voucher_info['date_added']) + ($this->config->get('cashback_valid')) * 86400;
				$date_now = time();
				
				if ($date_now > $date_expired || $voucher_info['to_email'] <> $this->customer->getEmail()) {
					$status = false;
				}
			}
		} else {
